...
const TradeRegister = () => {
  ...
  const addTradeRegistration = async (registrationData) => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to add trade registration');
      }
      await fetchTradeRegisters();
      return true;
    } catch (err) {
      setError(err.message);
      return false;
  ...
  const updateTradeRegistration = async (id, registrationData) => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to update trade registration');
      }
      await fetchTradeRegisters();
      return true;
    } catch (err) {
  ...
  const deleteTradeRegistration = async (id, recordDate) => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to delete trade registration');
      }
      await fetchTradeRegisters();
      return true;
    } catch (err) {
  ...
  const filteredAndSortedRegistrations = tradeRegisters
    .filter(registration => {
      ...
      const tradeMatch = !filters.tradeCode || registration.Trade_Code === filters.tradeCode;
      ...
      return workerMatch && deptMatch && tradeMatch && statusMatch && dateMatch && validityAlertMatch;
    })
    .sort((a, b) => {
      ...
      if (sortConfig.key === 'Record_Date' || sortConfig.key === 'Enrollment_Date') {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingRegistration) {
      ...
      success = await updateTradeRegistration(editingRegistration.Worker_ID, updateData);
    } else {
      success = await addTradeRegistration(formData);
    }
  ...
  setFormData({
    ...
    Enrollment_Date: registration.Enrollment_Date ? new Date(registration.Enrollment_Date).toISOString().split('T')[0] : '',
  ...
  const handleDelete = async (id, recordDate) => {
    if (window.confirm('Are you sure you want to delete this trade registration?')) {
      await deleteTradeRegistration(id, recordDate);
    }
  };
  ...
  if (loading) {
    return <div className="content-area">Loading trade registrations...</div>;
  }
  ...
  return (
    <div className="content-area">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h2 style={{ color: '#8B4513' }}>Trade Register Management</h2>
        <button 
        ...
        if (overdueCount > 0 || expiringCount > 0) {
          return (
            <div style={{ 
              ...
              <h4 style={{ color: '#856404', marginBottom: '0.5rem' }}>⚠️ Validity Alerts</h4>
              <div style={{ display: 'flex', gap: '2rem', fontSize: '0.9rem' }}>
                {overdueCount > 0 && (
                  <span style={{ color: '#721c24', fontWeight: '600' }}>
                    🔴 {overdueCount} registration(s) overdue
                  </span>
                )}
                {expiringCount > 0 && (
                  <span style={{ color: '#856404', fontWeight: '600' }}>
                    🟡 {expiringCount} registration(s) expiring within 7 days
                  </span>
                )}
              </div>
            </div>
      ...
      <div style={{ 
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          <span style={{ color: '#6c757d', fontSize: '0.9rem' }}>
            Showing {filteredAndSortedRegistrations.length} of {tradeRegisters.length} registrations
          </span>
        </div>
      </div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">
                {editingRegistration ? 'Edit Trade Registration' : 'Add New Trade Registration'}
              </h3>
              ...
              <div className="form-row full-width">
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    placeholder="Additional notes about the trade registration"
                  />
                </div>
              </div>
      ...
      <div className="table-container">
        <table className="table">
          <thead>
            <tr>
              ...
              <th 
                ...
                style={{ cursor: 'pointer', userSelect: 'none' }}
              >
                Enrollment Date {sortConfig.key === 'Enrollment_Date' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
              </th>
              <th 
          ...
          <tbody>
            {tradeRegisters.length === 0 && Object.values(filters).every(v => !v) ? null :
              filteredAndSortedRegistrations.length === 0 ? (
                <tr><td colSpan="9" style={{ textAlign: 'center', color: '#A0522D' }}>Trade registration not found</td></tr>
              ) : (
                currentRecords.map(registration => (
                  <tr key={`${registration.Worker_ID}-${registration.Record_Date}`}>
                    ...
                    <td>{registration.Trade_Code}</td>
                    <td>{new Date(registration.Enrollment_Date).toLocaleDateString()}</td>
                    <td>
